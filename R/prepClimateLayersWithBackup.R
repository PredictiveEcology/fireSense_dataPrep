...
#' @param tag character. Defaults to "Calc" (used for fireSense). Useful to run this for a new studyArea without
...
#' @param checkFutureLayers logical. Defaults to FALSE. If TRUE, prints the difference between 
#'                          future 2100 and 2011 layers for 4 variables: PPT_wt, Tave_sm, MSP, DD18;
#'                          Only happens in 2011.   
#'
...
#'
#' @importFrom usefulFuns grepMulti.R
#' @rdname prepClimateLayersWithBackup
prepClimateLayersWithBackup <- function(pathInputs = NULL,
                                           variables = NULL,
                                           years = NULL,
                                           GDriveFolder = NULL,
  ...
  yearsList <- lapply(X = years, FUN = function(y) {
    ...
    if (all(file.exists(paste0(file_path_sans_ext(fileName), ".zip")),
      ...
      message(yellow(paste0(fileName, " does not exist locally or should be overwritten. ",
                            "Checking the cloud... ")))
      filesInFolder <- googledrive::drive_ls(path = as_id(GDriveFolder), recursive = FALSE)
      if (all(paste0(basename2(file_path_sans_ext(fileName)), ".zip") %in% filesInFolder$name,
       ...
       reproducible::preProcess(url = paste0("https://drive.google.com/open?id=", filesInFolder$id[rw]),
        ...
        if (any(!file.exists(file.path(pathInputs, fullDatasetName)),
          ...
          checkPath(pathInputs, create = TRUE)
          googledrive::drive_download(file = climateFilePath, 
                                      path = file.path(pathInputs, fullDatasetName))
          if (tools::file_ext(file.path(pathInputs, fullDatasetName)) == "zip"){
            system(paste0("unzip ", file.path(pathInputs, fullDatasetName), " -d ", pathInputs))
          } else {
            if (tools::file_ext(file.path(pathInputs, fullDatasetName)) == "7z"){
              system(paste0("7za x ", file.path(pathInputs, fullDatasetName)))   
            } else {
        ...
        folders <- setdiff(list.dirs(path = pathInputs), datasetsPath)
        ...
        variablesStack <- stack(lapply(X = filesToLoad, FUN = function(variable){
          message(paste0("Processing ", variable, "..."))
          ras <- raster(x = file.path(currentYearsFolder, variable))
          if (!is(ras, "Raster")) browser()
          crs(ras) <- sp::CRS(paste0('+init=epsg:4326 +proj=longlat +ellps=WGS84 ",
                                  "+datum=WGS84 +no_defs +towgs84=0,0,0'))
          # Test the file is ok
          
          rasOk <- tryCatch({
            message(paste0("Testing ", variable, "'s integrity"))
            rastesting <- reproducible::projectInputs(x = ras,
                                                     targetCRS = crs(rasterToMatch),
                                                     method = "bilinear",
                                                     filename2 = NULL)
            message(crayon::green(paste0(variable, ": OK")))
            TRUE
          }, error = function(e){
            warning(paste0(e, " - Error converted to warning. ",
                           "The raster will be replaced by average ",
                           "of previous and following year"), 
                    immediate. = TRUE)
            return(FALSE)
          })
          if (!rasOk){
            currentYearsFolder1 <- grepMulti(x = folders, 
                                             patterns = c(climateModel, RCP,
                                                          ensemble, paste0(y-1)))
            currentYearsFolder2 <- grepMulti(x = folders, 
                                             patterns = c(climateModel, RCP,
                                                          ensemble, paste0(y+1)))
            r1 <- raster(x = file.path(currentYearsFolder1, variable))
            r2 <- raster(x = file.path(currentYearsFolder2, variable))
            message(paste0("Calculating average ", variable, " for ", y))
            ras <- raster::calc(raster::stack(r1, r2),
                                fun = mean, na.rm = TRUE)
            crs(ras) <- sp::CRS(paste0('+init=epsg:4326 +proj=longlat +ellps=WGS84 ",
                                  "+datum=WGS84 +no_defs +towgs84=0,0,0'))
          }
          if (any(!is.null(rasterToMatch),
                  !is.null(studyArea))){
            message(paste0("RTM or studyArea detected. Reprojecting ", variable))
            rasReproj <- reproducible::projectInputs(x = ras,
                                                   targetCRS = crs(rasterToMatch),
                                                   method = "bilinear")
            message(paste0("RTM or studyArea detected. Postprocessing ", variable))
            ras <- postProcess(rasReproj,
                                   destinationPath = pathInputs,
        ...
        names(variablesStack) <- file_path_sans_ext(filesToLoad)
        ...
        if (all(checkFutureLayers, years == 2011)) {
          print("Checking future layers for visual patterns...")
          runSquarenessTest(climateModel = climateModel, 
                            ensemble = ensemble)
          }
